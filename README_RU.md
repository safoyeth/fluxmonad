...
- [Архитектура и внутреннее устройство](docs/architecture.md): Граф выполнения, законы монад и модели работы с памятью.
- [Оптимизация и проталкивание предикатов](docs/optimization.md): Rule-based оптимизатор, слияние узлов и проверка безопасности зависимостей.
- [Бенчмарки производительности](docs/benchmarks.md): Методология измерений, телеметрия `tracemalloc` и показатели масштабируемости.
- [Полный справочник API](docs/api_reference.md): Все методы `Flux`, параметры, операторы и выражения.
